...
function pgraphics_draw_torii(pg, post_height, post_base_width, post_width, post_angle, crossbar_width_ratio, crossbar_height, crossbar_offset, has_crossbar_middle, top_style, top_thickness, top_layers, top_decorations, has_base, base_height, base_sock_height, base_width, base_base_color, base_color, accent_color) {
    ...
    if (has_crossbar_middle) {
        pg.fill(base_color);
        pg.beginShape();
        pg.vertex(0 - crossbar_height / 2, -post_height + crossbar_offset);
        pg.vertex(0 - crossbar_height / 2, -post_height * 1.02);
        pg.vertex(0 + crossbar_height / 2, -post_height * 1.02);
        pg.vertex(0 + crossbar_height / 2, -post_height + crossbar_offset);
        pg.endShape(CLOSE);
    }
    ...
    pg.fill(base_color);
    pg.beginShape();
    pg.vertex(crossbar_tops[0][0], crossbar_tops[0][1]);
    pg.vertex(crossbar_tops[0][0], crossbar_tops[0][1] - crossbar_height);
    pg.vertex(crossbar_tops[1][0], crossbar_tops[1][1] - crossbar_height);
    pg.vertex(crossbar_tops[1][0], crossbar_tops[1][1]);
    pg.endShape(CLOSE);
    ...
    for (let i = 0; i < 2; i++) {
        pg.fill(base_color);
        pg.beginShape();
        pg.vertex(post_bottoms[i][0] - post_width / 2, post_bottoms[i][1]);
        pg.vertex(post_tops[i][0] - post_width / 2, post_tops[i][1]);
        pg.vertex(post_tops[i][0] + post_width / 2, post_tops[i][1]);
        pg.vertex(post_bottoms[i][0] + post_width / 2, post_bottoms[i][1]);
        pg.endShape(CLOSE);
    }
    ...
    if (has_base) {
        for (let i = 0; i < 2; i++) {
            pg.fill(base_base_color);
            pg.beginShape();
            pg.vertex(post_bottoms[i][0] - base_width / 2, post_bottoms[i][1]);
            pg.vertex(post_bottom_tops[i][0] - base_width / 2, post_bottom_tops[i][1]);
            pg.vertex(post_bottom_tops[i][0] + base_width / 2, post_bottom_tops[i][1]);
            pg.vertex(post_bottoms[i][0] + base_width / 2, post_bottoms[i][1]);
            pg.endShape(CLOSE);
        }
        ...
        if (base_sock_height != null) {
            for (let i = 0; i < 2; i++) {
                ...
                pg.beginShape();
                pg.vertex(post_bottoms[i][0] - post_width / 2, post_bottoms[i][1]);
                pg.vertex(post_bottom_sock_tops[i][0] - post_width / 2, post_bottom_sock_tops[i][1]);
                pg.vertex(post_bottom_sock_tops[i][0] + post_width / 2, post_bottom_sock_tops[i][1]);
                pg.vertex(post_bottoms[i][0] + post_width / 2, post_bottoms[i][1]);
                pg.endShape(CLOSE);
            }
    ...
    if (top_style == 'flat') {
        ...
        pg.beginShape();
        pg.vertex(top_pts[0][0], top_pts[0][1]);
        pg.vertex(top_pts[0][0] - extension_width, top_pts[0][1] - top_thickness);
        pg.vertex(top_pts[1][0] + extension_width, top_pts[1][1] - top_thickness);
        pg.vertex(top_pts[1][0], top_pts[1][1]);
        pg.endShape(CLOSE);
    } else if (top_style == 'flat_curve') {
        const extension_width = -top_pts[0][0] * 0.08;
        const num_points = 50;
        ...
        // TODO: figure out how to fix the corner bug
        pg.push();
        pg.translate(0, lerp(crossbar_tops[0][1], top_pts[0][1], 0.8));
        for (let i = 0; i < top_layers; i++) {
            pg.curveTightness(1);
            ...
            pg.beginShape();
            pg.vertex(-cross_width / 2, 0);
            // Exact vertex caused weird corner artifacts
            pg.vertex(-cross_width / 2 - extension_width, -cross_height * 1.99);
            pgraphics_arc_vertices(pg, cross_width + extension_width * 2, cross_height, 0, -cross_height, num_points, 'right', 'full');
            // vertex(cross_width / 2 + extension_width, -cross_height);
            pg.vertex(cross_width / 2, 0);
            pg.endShape(CLOSE);
            pg.translate(0, -cross_height * 0.6);
            cross_width *= 1.1;
            cross_height *= 1.1;
        }
        pg.pop();
        ...
        pg.push();
        // pg.curveTightness(1.3);
        pg.translate(0, lerp(crossbar_tops[0][1], top_pts[0][1], 0.9));
        for (let i = 0; i < top_layers; i++) {
            if (i == (top_layers - 1)) {
                pg.fill(base_base_color);
            } else {
            ...
            pg.beginShape();
            pgraphics_arc_vertices(pg, cross_width, cross_height * 0.5, 0, 0, num_points, 'left', 'full');
            pgraphics_arc_vertices(pg, cross_width, cross_height * 0.5, 0, -cross_height, num_points, 'right', 'full');
            pg.endShape(CLOSE);
            pg.translate(0, -cross_height * 1.0);
            cross_width *= 1.1;
            cross_height *= 1.1;
        }
        pg.pop();
...
function pgraphics_arc_vertices(pg, w, h, w_offset, h_offset, num_points, direction = 'left', arc_type = 'full', center_out = true) {
